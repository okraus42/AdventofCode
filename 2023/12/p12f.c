...
/*   Updated: 2023/12/27 17:57:27 by okraus           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ok_checknumbers2(char str[500], int numbers[100], int s, int num, long long array[100][500])
{
	...
	if (str[i] == '?')
	{
		...
		i1 = i + 1;
		n1 = n;
		ok_checknumbers2(str, numbers, i1, n1, array);
		r += array[n1][i1];
		//ft_printf(".array[%i][%i] = %lli, str %s, num %i\n", n1, i1, array[n1][i1], &str[i1], numbers[n1]);
		h = numbers[n];
		while (h)
		{
			if (!str[i] && h)
		...
		if (str[i] && str[i] != '#')
		{
			i2 = i + 1;
			n2 = n;
			ok_checknumbers2(str, numbers, i2, n2, array);
			r += array[n2][i2];
			//ft_printf("#array[%i][%i] = %lli, str %s, num %i\n", n2, i2, array[n2][i2], &str[i2], numbers[n2]);
		}
		array[num][s] = r;
	}
	else
	...
	//ft_printf("F.array[%i][%i] = %lli, str %s, num %i\n", n1, i1, array[n1][i1], &str[i1], numbers[n1]);
	//ft_printf("F#array[%i][%i] = %lli, str %s, num %i\n", n2, i2, array[n2][i2], &str[i2], numbers[n2]);
	//ft_printf("FFarray[%i][%i] = %lli, str %s\n", num, s, array[num][s], &str[s]);
}
